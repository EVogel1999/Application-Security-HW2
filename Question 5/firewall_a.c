...
unsigned int hook_func(
    ...
    if (skb) {
        ...
        if (iph && iph->protocol == 6) {
            ...
            if (source_port == 23) {
	            printk(KERN_INFO "firewall_a.c -- Dropping TelNet TCP packets\n");
                return NF_DROP;
            }
        }
            ...
            if (source_port == 23) {
	            printk(KERN_INFO "firewall_a.c -- Dropping TelNet UDP packets\n");
                return NF_DROP;
            }
        }
    }    
...
int init_module() {
    ...
    printk(KERN_DEBUG "firewall_a.c -- Module loaded\n");  
    return 0;
}

void cleanup_module() {
    nf_unregister_hook(&nfho);
    printk(KERN_DEBUG "firewall_a.c -- Module unloaded\n");
}